...
const char ESP_FIRMWARE[] = "1.1.0";
AsyncMqttClient mqtt;
Uptime espUptime;
LineReader<240> uartLine;
char onlineTopic[128], statusCommand[128], purgeCommand[128];
char clientId[32];
...
void receiveLine(char* line) {
  ...
  if (sscanf(line, "STAT uptime=%11s state=%11s count=%11s duration_ms=%11s success=%7s firmware=%19s epoch=%11s time_valid=%2s%n",
    ...
    if (haveController && (newUptime < unoUptime || newCount < count)) unoClockValid = false;
...
void onMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties,
  ...
  // No retained, fragmented, large, or burst command execution.
  if (properties.retain || index || len != total || total != 7 ||
      memcmp(payload, "request", 7) || uint32_t(millis() - lastCommand) < 5000) return;
  lastCommand = millis();
  if (!strcmp(topic, statusCommand)) requestStatus = true;
  // command/purge is recognized but disabled at BOTH gateway and Uno.
  // Future: validate authorization, freshness and request ID, then send CMD purge.
  // The Uno must enforce all local startup/busy/sensor interlocks.
}
...
void publishNext() {
  ...
  switch (topicIndex) {
    ...
    case 21: suffix="command/purge_enabled"; value="false"; break;
  ...
  if (inFlight) topicIndex = (topicIndex + 1) % 29;
}
...
void setup() {
  ...
  mqtt.onDisconnect([](AsyncMqttClientDisconnectReason) {
    connecting=false; inFlight=0; lastConnect=millis();
  });
